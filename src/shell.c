...
char ***split_double_char_on_string(char **input, char *split_string) {
  ...
  for (int i = 0; input[i] != NULL; i++) {
    ...
    strcat(final[num_splits][input_count], input[i]);
...
char ***split_double_char_on_string_list(char **input, char **split_string) {
  ...
  for (int i = 0; input[i] != NULL; i++) {
    ...
    strcat(final[num_splits][input_count], trim(input[i]));
    final[num_splits][input_count][(int)strlen(trim(input[i]))] = '\0';
    input_count++;
  }
...
void free_memory_kill_children() {
  ...
  if (running_jobs != NULL) {
    for (int i = 0; i < job_count; i++) {
      ...
      int status;
      pid_t return_pid = waitpid(running_jobs[i].pid, &status, WNOHANG);
      if (return_pid == 0) {
...
int launch_job_op(char **command, int command_count, bool background) {
  ...
  char *ops[3] = {"&&\0", "||\0", NULL};
  ops[0] = (char *)malloc(sizeof(char) * 3);
  ops[1] = (char *)malloc(sizeof(char) * 3);
  ops[2] = NULL;
  ops[0] = "&&\0";
  ops[1] = "||\0";
