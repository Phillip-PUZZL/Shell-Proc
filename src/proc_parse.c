...
void init_memory() {
  ...
  memset(cont_data.sectors_read_per_second, 0.0, sizeof(float) * drive_count);
  memset(cont_data.sectors_written_per_second, 0.0,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (i == 0) {
      ...
      while (fgets(read_buffer, sizeof(read_buffer), info_file)) {
        ...
        } else if (strncmp(read_buffer, "MemFree", (int)strlen("MemFree")) ==
                   0) {
          // Free memory on this line, grab it
          sscanf(read_buffer, "MemFree: %ld %*s", &free_memory);
        }
      }
      ...
      while (fgets(read_buffer, sizeof(read_buffer), info_file)) {
        if (sscanf(read_buffer,
          ...
          cont_data.sectors_read_per_second[index] +=
              ((float)(sectors_read -
                       (float)cont_data.prev_sectors_read[index]) /
               (float)read_rate);
          cont_data.sectors_written_per_second[index] +=
              ((float)(sectors_written -
                       (float)cont_data.prev_sectors_written[index]) /
               (float)read_rate);
